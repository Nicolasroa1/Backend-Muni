...
public interface FirmaRepo extends JpaRepository<OrdenDeCompra, Long> {
    ...
    @Modifying
    @Query(value = "UPDATE public.orden_de_compra SET estado = 'estado_primera_firma', firmas = array_append(firmas, 'QRFIRMA1') WHERE orden_de_compra_producto_solicitud_id = ?1 AND estado = 'pendiente'", nativeQuery = true)
